...
for update in range(1000000):
    ...
    if update % (cfg['environment']['eval_every_n'] * n_steps) == 0:
        ...
        loaded_graph = Actor(ob_dim, cfg['architecture']['policy_net'], act_dim).to(device)
        loaded_graph.load_state_dict(torch.load(saver.data_dir+"/full_"+str(update)+'.pt')['actor_state_dict'])
        loaded_graph.training = False

        env.turn_on_visualization()
        env.start_video_recording(saver.data_dir + "/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + "_policy_"+str(update)+'.mp4')

        for step in range(n_steps*2):
            frame_start = time.time()
            obs = env.observe(False)
            action_ll, _ = loaded_graph(torch.from_numpy(obs).cpu())
